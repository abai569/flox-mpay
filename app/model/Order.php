...
class Order extends BaseModel
{
    ...
    {
        $select = [];
        $allow_field = ['id', 'order_id', 'pid', 'type', 'out_trade_no', 'name', 'really_price', 'money', 'state', 'create_time_start', 'create_time_end', 'close_time', 'pay_time', 'pay_source', 'platform', 'platform_order', 'aid', 'cid',];
    ...
    private static function checkMoney($money, $type, $aid, $cid, $chan): float
    {
        ...
        // 整数金额使用 0.01-0.08 的随机尾差区分同时支付的订单。
        if (abs($money - round($money)) < 0.000001) {
            $usedPrices = [];
            foreach ($activeOrders as $activePrice) {
                $usedPrices[] = number_format((float) $activePrice, 2, '.', '');
            }

            $candidates = [];
            for ($cent = 1; $cent <= 8; $cent++) {
                $candidate = number_format($money + ($cent / 100), 2, '.', '');
                if (!in_array($candidate, $usedPrices, true)) {
                    $candidates[] = $candidate;
                }
            }
            if ($candidates) {
                return (float) $candidates[random_int(0, count($candidates) - 1)];
            }

            // 同一通道最多只能同时区分八笔同整数金额订单。
            return $money + (random_int(1, 8) / 100);
        }
