...
class PayController
{
    ...
    public function mapi(Request $request)
    {
        ...
        // 检查商户订单
        $existing_order = Order::where('pid', $req_data['pid'])->where('out_trade_no', $req_data['out_trade_no'])->find();
        if ($existing_order) return json($this->buildMapiResponse($request, $existing_order));
        // 创建新订单
        $order_info = Order::createOrder($req_data);
        if ($order_info['code'] !== 0) return json(backMsg(0, $order_info['msg']));
        $order = Order::where('order_id', $order_info['data']['order_id'])->find();
        return json($this->buildMapiResponse($request, $order));
    }

    private function buildMapiResponse(Request $request, Order $order): array
    {
        $channel = PayChannel::where('id', $order->cid)->find();
        $payCode = $channel ? $channel->qrcode : '';
        if ($channel && preg_match('/^alipay4#\d+$/', $channel->channel)) {
            $result = \payclient\AliPayf::getPayUrl($order->order_id, $order->money, $channel->qrcode);
            $payCode = $result['data'] ?? $result['msg'] ?? '';
        }
        ...
        return [
            ...
            'payurl' => $request->domain() . "/Pay/console/{$order->order_id}",
            'pay_code' => $payCode,
            'code_type' => $channel ? (int) $channel->type : 0,
            'really_price' => (string) $order->really_price,
            'expires_at' => strtotime($order->close_time),
        ];
    }
    // 收银台
        ...
        if ($order_id) {
            ...
            if ($act_order) {
                ...
                if (preg_match('/^alipay4#\d+$/', $channel->channel)) {
                    ...
                    // 支付宝账单模式：生成APP唤起链接（仅传收款人，不传金额和备注，避免风控）
    ...
    private function payHeart(array $records, array $config)
    {
        ...
        if (!count($new_orders)) return ['code' => 0, 'msg' => '收款通知无新消息'];
        // 有效订单列表
        $activeOrders = Order::scope('activeOrder')->where($query)->select();
        if (!count($activeOrders)) return ['code' => 0, 'msg' => '数据库无有效期订单'];
        // 查找所有支付渠道
        ...
        foreach ($new_orders as $new_order) {
            foreach ($activeOrders as $order) {
                // 支付方式核对
                $is_payway = strcasecmp((string)$order->type, (string)$new_order['payway']) === 0;
                if ($new_order['payway'] == '') $is_payway = true;
                // 支付渠道核对
                $is_channel = empty($new_order['channel']) || ((isset($cids[$order->cid]) ? $cids[$order->cid] : '') == $new_order['channel']);
                // 金额核对
        ...
        if (!$notify) return ['code' => 0, 'msg' => '收款通知无匹配订单'];
        return $notify;
    }
    // 修改订单状态并通知
    ...
    {
        // 支付成功
        $set_order_state = $order->save(['state' => 1, 'pay_time' => date('Y-m-d H:i:s', time()), 'pay_source' => 'auto', 'platform_order' => $order_no]);
        if (!$set_order_state) {
        ...
        if (empty($order->notify_url)) {
            return ['order' => $order->order_id, 'code' => 0, 'msg' => 'notify fail: notify_url empty'];
        }
        // 异步通知
        $res_notify = self::getHttpResponse($order->notify_url, [], http_build_query($notify));
        if (in_array(strtolower(trim((string)$res_notify)), ['success', 'ok'])) {
            return ['order' => $order->order_id, 'code' => 1, 'msg' => 'notify success'];
        } else {
            $preview = mb_substr(trim((string)$res_notify), 0, 120);
            if ($preview === '') $preview = 'empty response';
    ...
    public function checkOrder($pid = '', $sign = '')
    {
        ...
        if ($is_user) {
            $orders = Order::scope('activeOrder')
                ->where('pid', $pid)
                ->field('id,pid,aid,cid,patt,type,really_price')
                ->select();
            $old_info = cache('order');
            $num = count($orders);
            if ($num > 0) {
                $info = ['code' => 1, 'msg' => "有{$num}个新订单", 'orders' => $orders];
    ...
    public function mpayNotify(Request $request)
    {
        ...
        if (!isset($data['pid'])) return 'pid参数错误';
        $config = false;
        if (isset($data['aid']) && $data['aid'] !== '') {
            $config = PayAccount::getAccountConfig($data['aid'], $data['pid']);
        }
        if ($config !== false && !empty($config['payclass'])) {
            $payclient_file = root_path() . '/extend/payclient/' . $config['payclass'] . '.php';
            if (file_exists($payclient_file)) {
                ...
                $result = $this->payHeart($res['data'], $config);
                if (isset($result['code'])) {
                    return $result['code'] === 1
                        ? json(['code' => 1, 'msg' => 'OK', 'order_id' => $result['order'] ?? ''])
                        : json($result);
                }
                foreach ($result as $item) {
                    if (isset($item['code']) && $item['code'] === 1)
                        return json(['code' => 1, 'msg' => 'OK', 'order_id' => $item['order'] ?? '']);
                }
                return json(['code' => 0, 'msg' => '收款通知处理失败']);
            }
            // 插件文件不存在，mpaypc 走直推 fallback
            if ($action !== 'mpaypc') return '监听客户端文件不存在';
        }
        if ($action !== 'mpaypc') return '监听配置错误';
        if (empty($data['price']) || empty($data['payway'])) return 'price或payway参数缺失';
        $records = [[
            'order_no' => 'PC_' . time(),
            'price' => (string)$data['price'],
            'payway' => (string)$data['payway'],
            'channel' => '',
        ]];
        $result = $this->payHeart($records, ['pid' => (string)$data['pid']]);
        if (isset($result['code'])) {
            return $result['code'] === 1
                ? json(['code' => 1, 'msg' => 'OK', 'order_id' => $result['order'] ?? ''])
                : json($result);
