...
class OrderController extends BaseController
{
    ...
    // 批量手动补单
    public function batchPayOrder()
    {
        $ids = $this->request->post('ids');
        if (!$ids || !is_array($ids)) {
            return json(\backMsg(1, '请选择需要补单的订单'));
        }
        ...
        if ($success > 0 && $notify_fail === 0 && $not_found === 0) {
            return json(\backMsg(0, '批量补单成功，共' . $success . '单'));
        }
        if ($success > 0) {
            return json(\backMsg(0, '补单成功' . $success . '单，失败' . $notify_fail . '单，不存在' . $not_found . '单'));
        }
        return json(\backMsg(1, '批量补单失败'));
    }
    // 重新通知
