...
适用于 Linux 服务器（Ubuntu / Debian / CentOS 等），自动安装 Docker 并启动服务：

```bash
bash <(curl -L https://raw.githubusercontent.com/abai569/mpay-flvx/main/install.sh)
```

选择 **1. 安装 mpay**，按提示设置端口（默认 8088）和域名（可选）即可完成部署。

安装完成后访问 `http://你的IP:端口/install` 完成 Web 安装向导。

### Docker 镜像

支持 `linux/amd64` 和 `linux/arm64` 双架构，自动从 GitHub Container Registry 拉取：
...
| 层级 | 技术 |
|---|---|
| 后端 | ThinkPHP 8（PHP 8.0+，推荐 8.2） |
| 前端 | Layui 2.9 + PearAdmin 后台 |
| 数据库 | SQLite（内置，零配置） |
| Web 服务器 | Nginx（Docker 内置） |
| 运行环境 | Docker（支持 amd64 / arm64） |
...
适用于任何 Linux 发行版，无需手动安装 PHP / Nginx / MySQL：
...
1. 运行脚本，选择 **1. 安装 mpay**
2. 输入访问端口（默认 **8088**）
3. 可选配置域名反代（自动申请 SSL 证书）
4. 等待 Docker 镜像构建完成（首次约 5-10 分钟）
5. 访问 `http://你的IP:端口/install` 完成 Web 安装

> SQLite 数据库文件自动通过 Docker 卷持久化，重启不丢失。
...
从 [Releases](https://github.com/abai569/mpay-flvx/releases) 下载源码压缩包并上传到网站目录解压
...
浏览器访问 `http://你的域名/install`，进入程序安装界面，按提示填写数据库等信息完成安装。

> 宝塔方式需要自行安装 PHP 8.0+ 和 MySQL 5.7+，推荐使用 Docker 方式更简单。
