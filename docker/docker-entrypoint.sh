...
if [ "$_DB_TYPE" = "sqlite" ] && [ -f /var/www/html/database/mpay.db ]; then
    php /var/www/html/docker/migrate_db.php 2>&1 || true
fi
...
# Keep runtime/session intact across container recreates; clearing the whole
# runtime directory logs users out after every update.
rm -rf /var/www/html/runtime/cache/* /var/www/html/runtime/temp/* 2>/dev/null || true
